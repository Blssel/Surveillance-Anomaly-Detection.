...
def _parse_function(filename):
  ...
  feature_list=glob.glob(os.path.join(feature_path,'*'))
  ##读取该视频对应的特征文件（很多个）
  ##seg=np.array([])
...
def _ano_generator():
  ''' ano生成器  '''
  split_list_ano, _ = _read_split(Args.train_split_path) #从txt文件中读入ano split列表
  feature_bag=[] # 定义容器，存放该视频的所有feature
  #---逐个读取文件----#
  for filename in split_list_ano:
    feature_path=os.path.join(Args.feature_path , filename.split('.')[0])
    feature_list=glob.glob(os.path.join(feature_path,'*'))
    # 逐个读取该视频对应的特征文件
    for index in range(0,len(feature_list)-1,2):
